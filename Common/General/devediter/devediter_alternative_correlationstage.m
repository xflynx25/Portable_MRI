function [win_stack] = devediter_alternative_correlationstage(combined_data, starter_kern_stack, correlation_eps, mingroupsize)
    ...
    window_size = 1;
    max_window_size = nlin - clin + 1; % Maximum possible window size from 'clin' to end
    pe_rng = clin; % Start with the current line
    
    % Iterate to expand the window as much as possible without encountering zeros
    while window_size <= max_window_size
        current_window = clin : (clin + window_size -1);
        
        % Extract the sub-correlation matrix for the current window
        sub_corr = kcor_thresh(current_window, current_window);
        
        % Set diagonal to 1 to ignore self-correlation
        sub_corr(logical(eye(length(current_window)))) = 1;
        
        % Check if there are any zeros in the sub-correlation matrix
        if any(sub_corr(:) == 0)
            % If a zero is found, stop expanding the window
            if window_size > 1
                % Assign the previous window as the valid group
                pe_rng = clin : (clin + window_size - 2);
            else
                % If window_size is 1 and a zero is found, assign only 'clin' to the group
                pe_rng = clin;
            end
            break; % Exit the expansion loop
        else
            % If no zero is found, expand the window
            pe_rng = current_window;
            window_size = window_size + 1;
        end
    end
    
    % If the window was expanded to the maximum possible size without encountering zeros
