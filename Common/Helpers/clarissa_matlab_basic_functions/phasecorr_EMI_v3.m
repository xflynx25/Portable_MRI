...
% 
% anglecheck = angle(Ms_pcorr_pos(57,nphase2,1));
% disp(anglecheck);
% 
% Ms_pcorr_pos = Ms_pcorr_pos.*exp( 1j * -anglecheck);
% anglecheck = angle(Ms_pcorr_pos(57,nphase2,1));
% disp(anglecheck);
...
I3Dfid1 = ifftshift(ifftn(ifftshift(dataFID(:,2:end,1:end-2))));
anglecheckimage = angle(I3Dfid1(ceil(end/2),ceil(end/2),ceil(end/2)));
dataFID = dataFID.*exp( 1j * -anglecheckimage);

% anglecheckFID = angle(dataFID1(132,1,29));
% disp(anglecheckFID);disp(anglecheckimage);
