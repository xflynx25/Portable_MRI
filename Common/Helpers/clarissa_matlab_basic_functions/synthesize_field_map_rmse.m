function [rmse, field_map_check, C] = synthesize_field_map_rmse(XX,YY,trajectory_all, field_all, N_order)
...
field_map_check = trajectory_all(:,1);    %initialize field map
coeff_num = 1;                  %intialize variable
for y_order=0:(N_order)
  for x_order=0:(N_order)
    if x_order+y_order<(N_order+1)
       field_map_check = field_map_check + C(coeff_num)*trajectory_all(:,1).^x_order.*trajectory_all(:,2).^y_order;
       coeff_num = coeff_num+1;
    end
  end
end
...
rmse = sqrt(mean((field_all - field_map_check).^2));
