function plot_emi_mitigation_slider(combined_data, N, slice_type, square_plot, scale_param)
    ...
    emi_accumulator = zeros(size(primary_data));
    for detector = 2:size(combined_data, 4)
        detector_data = combined_data(:, :, :, detector);
        emi_transformed = abs(cartesian_3d_ifft(detector_data)); 
        emi_accumulator = emi_accumulator + emi_transformed;
    end
    data2dEMI = emi_accumulator / (size(combined_data, 4) - 1);
    ...
    title = sprintf('MRI Slice Viewer - Scale Param: %g', scale_param);
    ...
    function updateSlices(startSlice, numSlices, ax, data2d, I3D_Editer, data2dEMI, square_plot, scale_param)
