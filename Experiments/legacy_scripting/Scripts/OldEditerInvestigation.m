...
exp_name1 = 'with_blanket_newgradON_doubleacquisition_test1'; 
%exp_name1 = 'with_blanketgrounded_newgradON_doubleacquisition_test1'; 
dataFilePath = fullfile(projectdata1, 'Processed/tnt',['tnt_preprocessed_data_', exp_name1, '_FORMATTED.mat']);
...
switch num_dimensions
    ...
    case 6 % repeats as well 
        if size(combined_data, 5) == 2 && AVERAGING == 0 % calibration without inplane averaging
            cd = squeeze(combined_data(:, :, SLICE, :, 2, REPEAT_NUMBER));
            calib_d = squeeze(combined_data(:, :, SLICE, :, 1, REPEAT_NUMBER));
        else
            if size(combined_data, 5) == 1
                cd = squeeze(combined_data(:, :, SLICE, :, 1, REPEAT_NUMBER));
                calib_d = cd; 
            else
